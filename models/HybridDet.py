"""Package containing a slightily modified version of Hybridnets: https://github.com/datvuthanh/HybridNets
"""
...
def UpConv(input_channels, output_channels, stride=2, kernel_size=3, padding=0) -> nn.Sequential:
    """Make a simple deconvolution block used to build the decoder"""
    return nn.Sequential(
        nn.ConvTranspose2d(input_channels, output_channels, kernel_size, stride, padding=padding, output_padding=1),
        nn.ReLU()
    )
...
class SegmentationHead(nn.Sequential):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, activation=None, upsampling=1, alt=False):
        if alt:
            up1 = UpConv(in_channels, in_channels, kernel_size=kernel_size)
            up2 = UpConv(in_channels, out_channels, kernel_size=kernel_size)
            super().__init__(up1, up2)
        else:
...
class HybridDet(nn.Module):
    def __init__(self, line_classes, area_classes, compound_coef=3, alt=False):
        super(HybridDet, self).__init__()
        ...
        self.line_decoder = self._make_seg_decoder(line_classes, alt=alt)
        self.area_decoder = self._make_seg_decoder(area_classes, alt=alt)
    ...
    def forward(self, inputs):
        ...
        features = (p2, p3, p4, p5, p6, p7)

        line_seg = self.line_decoder(features)
        area_seg = self.area_decoder(features)

        return line_seg, area_seg

    def _make_seg_decoder(self, num_classes, activation=None, alt=False):
        decoder = BiFPNDecoder(pyramid_channels=self.fpn_num_filters[self.compound_coef])
        ...
        self.initialize_decoder(decoder)
        self.initialize_head(head)

        return nn.Sequential(decoder, head)
